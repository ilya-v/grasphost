...
public:
    ...
    static  struct  start_event_t               {}                              start_event;
            struct  event_handler_not_found_ex  { std::string info; };
                            ...
                            void            start()                             { set_state(0); logging_on(); process_event(&start_event); }
    ...
    template <typename TEvent>  void process_event(const TEvent *e)
    {
        *log << "State " << get_state_name() << "\tEvent " << typeid(*e).name() << std::endl;
        ...
        if (!handler_found)
        {
            *log << "\tNo handler found for event" << std::endl;
            throw  event_handler_not_found_ex{ get_state_name() + ": " + typeid(e).name() };
        }
    }

    void logging_off()  { static std::ofstream dev_null; log = &dev_null; }
    void logging_on ()  { log = &std::clog; }
    std::ostream & log_str()    { return *log; }
